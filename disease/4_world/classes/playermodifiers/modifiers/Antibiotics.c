modded class AntibioticsMdfr: ModifierBase
{
	...
	override void OnTick(PlayerBase player, float deltaT)
	{
		...
		if (player.GetSingleAgentCount(RageAgents.RAGEVIRUS) <= 0)
			{
				return;
			}

			// This makes sure the virus infection amount shrinks if it's below Stage 1 infection.
		if (player.GetSingleAgentCountNormalized(RageAgents.RAGEVIRUS) < 120 && player.GetBleedingManagerServer() && player.GetBleedingManagerServer().m_BleedingSources.Count() <= 0)
		{
				player.InsertAgent(RageAgents.RAGEVIRUS, -0.05); // 
				if (player.GetSingleAgentCount(RageAgents.RAGEVIRUS) <= 0)
				{
					player.RemoveAgent(RageAgents.RAGEVIRUS);
				}
		}
	}
};